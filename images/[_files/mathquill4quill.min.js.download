...
window.mathquill4quill = function(dependencies) {
    ...
    function enableMathQuillFormulaAuthoring(quill, options) {
        ...
        function newMatrixButtons() {
            const matrices = options.matrices || [];
            let container = null;

            function applyMatrixButtonStyles(button) {
                button.setAttribute("class", "mathquill4quill-matrix-button");
            }

            function applyMatrixContainerStyles(container) {
                container.setAttribute("class", "mathquill4quill-matrix-container");
            }

            function createMatrixButton(element, mqField) {
                const displayMatrix = element[0];
                const operator = element[1];

                const button = document.createElement("button");
                button.setAttribute("type", "button");

                katex.render(displayMatrix, button, {
                    throwOnError: false
                });
                button.onclick = () => {
            ...
            return {
                render(mqField) {
                    if (container != null || matrices.length === 0) {
                        return;
                    }
                    ...
                    container = document.createElement("div");
                    applyMatrixContainerStyles(container);

                    matrices.forEach(element => {
                        const button = createMatrixButton(element, mqField);
                    applyMatrixButtonStyles(button);
                    container.appendChild(button);
                });
        ...
        const matrixButtons = newMatrixButtons();
